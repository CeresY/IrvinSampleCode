...
public class Trans2PinYin {
	 ...
	 * 
	 * @param str 字符
	 * @param len 返回每个字符几位字母，-1返回全部，n返回n个 
	 * @return
	 */
	public String convertAll(String str, int len) {
		String result = "";
		String strTemp = null;
				...
				for (int i = (pyvalue.length - 1); i >= 0; i--) {
					if (pyvalue[i] <= ascii) {
						if(len == -1) {
							...
							result += pystr[i].substring(0, len);
						}
						//System.out.println("else:"+pystr[i]);
						break;
					}
				}
			}
	 ...
	 * @return
	 */
	public static String trans2PinYin(String str, int len) {
		return Trans2PinYin.getInstance().convertAll(str,len);
	}
